...
export default function ArticleView({ params }: { params: { id: string } }) {
  ...
  const cleanWord = (rawWord: string): string => {
    ...
    // Passo 1: Normalização Unicode (NFD para separar acentos)
    const normalized = rawWord.normalize('NFD');
    
    // Passo 2: Remover símbolos e pontuação no início/fim
    const trimmed = normalized.replace(/^[\p{P}\p{S}\p{N}]+|[\p{P}\p{S}\p{N}]+$/gu, '');
    
    // Passo 3: Validar com regex específica do idioma
    const isValid = new RegExp(languageSettings.word_regex, 'u').test(trimmed);
  ...
  useEffect(() => {
    const loadBaseData = async () => {
      ...
      const lang = await db.languages
        .where('iso_639_1').equals(art.language)
        .first();
    ...
    loadBaseData().then((data) => {
      ...
      // Carregar TODAS as palavras do idioma, não só as do artigo
        ...
        const articleWordIds = art.word_ids.split('$').filter(Boolean).map(Number);
  ...
  useEffect(() => {
    ...
    const processText = () => {
      ...
      article.original.split(splitRegex).forEach(rawWord => {
        ...
        if (cleaned) frequencyMap.set(cleaned, (frequencyMap.get(cleaned) || 0) + 1);
      });
    ...
    processText();
  }, [article?.article_id, languageSettings?.text_splitting_regex, languageSettings?.word_regex]);
  ...
  const splitText = (text: string) => {
    ...
    try {
      const splitRegex = new RegExp(languageSettings.text_splitting_regex, 'gu');
      return text.split(splitRegex).filter(w => w.length > 0);
    } catch (e) {
      console.error('Regex inválida, usando fallback', e);
      return text.split(/[\p{Z}\p{P}\p{S}]+/gu).filter(w => w.length > 0);
    }
  };
    ...
    if (showMarkAll === 0) {
      ...
      try {
        await Promise.all(words.map(word => 
          db.words.update(word.id!, { comfort: 5 })
        ));
  ...
  const handleWordClick = async (rawWord: string) => {
    ...
    try {
      ...
      const existingWord = words.find(w => 
        w.name.localeCompare(cleanedWord, undefined, { sensitivity: 'base' }) === 0
      );
      ...
      if (existingWord) {
        ...
        if (!article.word_ids.includes(existingWord.id!.toString())) {
          ...
          setArticle(prev => prev ? { ...prev, word_ids: updatedIds } : null);
        }
        return;
      }
      ...
      await db.articles.update(article.article_id!, { word_ids: updatedArticleIds });
      setArticle(prev => prev ? { ...prev, word_ids: updatedArticleIds } : null);
        ...
        <div className="mb-6 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <span className="text-sm">
              Known words: {knownWords}/{totalWords} ({(knownWords/totalWords*100).toFixed(1)}%)
            </span>
            <button
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1 bg-white rounded-lg shadow-md p-6 border">
            ...
            >
              ...
              {article && (wordsToShow.map((word, index) => {
                //const cleanedWord = word.replace(/^[^a-zA-ZÀ-ÿ]+|[^a-zA-ZÀ-ÿ]+$/g, '');
                ...
                return (
                  <span
                    ...
                    className={`px-1 cursor-pointer rounded ${
                        comfort === 5 ? 'bg-green-100' :
                        comfort === 4 ? 'bg-blue-100' :
                        comfort === 3 ? 'bg-yellow-100' :
                        comfort === 2 ? 'bg-red-100' : 
                        'hover:bg-gray-100'
                      } ${
                        selectedWord && wordData?.id === selectedWord.id ? 'border-2 border-blue-500' : ''
                      }`}
                      ...
                      {[1, 2, 3, 4, 5].map(num => (
                        <button
                          ...
                          onClick={async () => {
                            ...
                            try {
                                await db.words.update(updatedWord.id!, { comfort: num });
                            } catch (error) {
